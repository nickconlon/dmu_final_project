...
plot_image(final_image,[],[],[p_x,p_y],[],filename_final,save_image)

hist_data = [parse(Int64,i) for i in chosen_idx]
correct_data = []
wrong_data = []
for i in hist_data
    if i in truth_data
...
p = plot(histogram([wrong_data,correct_data], bins=0:1:42), xticks=0:5:40)
